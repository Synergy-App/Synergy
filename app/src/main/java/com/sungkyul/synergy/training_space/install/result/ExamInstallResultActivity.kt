package com.sungkyul.synergy.training_space.install.result
...
class ExamInstallResultActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_exam_message_result)
        // SharedPreferences에서 결과를 가져옴
        val sharedPreferences: SharedPreferences =
            getSharedPreferences("PracticeInstallPrefs", Context.MODE_PRIVATE)

        val result1 = sharedPreferences.getBoolean("install_result", true)
        ...
        problemBtn.setOnClickListener {
            val intent = Intent(this, ExamGoogleProblem::class.java)
            startActivity(intent)
        }
    }
